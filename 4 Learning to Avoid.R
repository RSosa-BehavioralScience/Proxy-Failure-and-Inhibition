...
# ------------------------------------GRADIENT DESCENDING VERSION
#-----------------------------------------------------------------------------

# Description:
#   This script employs the same learning rule as the previous script, but 
#   simulations are conducted in an alternative environment. In this setup, 
#   cue density does not signal proximity to the goal; rather, a higher cue density 
#   indicates a greater distance from the goal. As a result, goal attainment 
#   still becomes more efficient through experience but via an opposed 
#   feedback mechanism.
...
lr_cue <- 0.03           #Learning rate for target cue
...
#Position blobs
blob_x <- rep(-0.5, num_blobs) #Cues radiating from the agent's initial position
blob_y <- rep(-0.5, num_blobs)
...
for (i in 1:loops) {
  ...
  prev_cue <- 30             #First cue intensity experienced is 30
  ...
  steps <- tibble(
    ...
    cue_int = numeric(),       #Cue intensity (number of blobs)
  ...
  #...linear-operator rule assuming that the target cue gains or 
  ...
  while (!path_cross_goal(previous_x, previous_y, current_x, current_y, goal_x, goal_y, goal_radius)) {
    ...
    #Record current step data (only in the last episode)
    if (i == loops) {
      new_step <- tibble(
        step = nrow(steps) + 1,
        x = current_x,
    ...
    #Add one step to the step counter
    k <- k + 1
  ...
  #Count the blobs at the agent's last recorded position
  #This value will be appended to the feedback path to compute gain acceleration
  inside_blobs_ <- sapply(1:num_blobs, function(i) {
    point_in_polygon(current_x, current_y, blobs[[i]])
  })
  inside_blob_count_ <- sum(inside_blobs_)
  
  #Calculate cue acceleration
  gain_acceleration <- diff(c(feedback_path, inside_blob_count_)) 
...
steps$cue_int <- c(steps$cue_int[-1], 0)
steps$cue_int <- c(steps$cue_int[-1], NA)
...
steps_descending <- steps_count_per_iteration
V_evol_descending <- V_evol
...
blob_plot_data_desc_e <- blob_plot_data %>%
  arrange(id) %>%  
  mutate(
...
rep_episode_desc_e <- bind_rows(new_row, rep_episode_desc_e)

#Plot blobs and agent position with goal trace
agent_path_desc_e <- ggplot(blob_plot_data_desc_e, aes(x = x, y = y, group = id)) +
  ...
  geom_path(data = rep_episode_desc_e, aes(x = x, y = y, group = 1), 
...
print(agent_path_desc_e)
#Recommended dimensions: 700 × 650 pixels

agent_path_desc_e <- agent_path_desc_e + 
  guides(fill = "none") + 
  theme(
...
#To generate the trajectory plot shown in Figure 2:
#Set loops <- 491 to capture the representative episode of early stage
#Execute until the simulation loop (stopping before the analysis section)
#Run the command below to store the agent's path

rep_episode_desc_l <- steps
...
#Clip blobs to the visible region [0, 10]
blob_plot_data_desc_l <- blob_plot_data %>%
  arrange(id) %>%  
  mutate(
    x = pmax(pmin(x, 10), 0),  #Constrain x within [0, 10]
    y = pmax(pmin(y, 10), 0)   #Constrain y within [0, 10]
...
#Combine the new row with existing data
rep_episode_desc_l <- bind_rows(new_row, rep_episode_desc_l)

#Plot blobs and agent position with goal trace
agent_path_desc_l <- ggplot(blob_plot_data_desc_l, aes(x = x, y = y, group = id)) +
  geom_polygon(aes(fill = "Cue"), alpha = 0.03, color = "gray90") +  
  geom_circle(aes(x0 = goal_x, y0 = goal_y, r = goal_radius), fill = "red4", alpha = 1) +
  #Add agent displacement path
  geom_path(data = rep_episode_desc_l, aes(x = x, y = y, group = 1), 
            color = "darkgreen", alpha = 1, size = 0.75) +
  scale_x_continuous(limits = c(0, 10), breaks = seq(0, 10, 1), expand = c(0, 0)) +
  scale_y_continuous(limits = c(0, 10), breaks = seq(0, 10, 1), expand = c(0, 0)) +
  theme_minimal() +
...
print(agent_path_desc_l)
#Recommended dimensions: 700 × 650 pixels

agent_path_desc_l <- agent_path_desc_l +
...
#Main plot of steps descending
sd <- ggplot(data = data.frame(Episodes = seq_along(steps_descending),
                               Steps = steps_descending), 
             aes(x = Episodes, y = Steps)) +
  geom_line() +
  labs(x = "Episodes",
       y = "Number of Steps to Attain Goal") +
  scale_y_continuous(breaks = c(c(0,50,100), c(2:11)*100)) +
  theme_minimal() +
...
#Inset plot of V_evol_descending
vd <- ggplot(data = data.frame(Episodes = seq_along(V_evol_descending),
                               Value = V_evol_descending), 
             aes(x = Episodes, y = Value)) +
  scale_y_continuous(limits = c(-1, 0)) +
  geom_hline(yintercept = 0, color = "black") +
  geom_vline(xintercept = 0, color = "black") +
  geom_point(color = "blue", alpha = 0.1, size = 2.5) +
  labs(x = "Episodes",
       y = "Associative Value") +
...
sNv_d <- sd + inset_element(vd, left = 0.65, bottom = 0.6, right = 0.85, top = 0.9)

print(sNv_d)

#Ensamble plot to create Figure 2

#Arrange the top row (4 plots)
F2_top_row <- agent_path_climb_e + agent_path_climb_l + agent_path_desc_e + agent_path_desc_l + 
  plot_layout(ncol = 4)

#Arrange the bottom row (2 plots)
F2_bottom_row <- sNv_c + sNv_d + 
  plot_layout(ncol = 2)

#Combine top and bottom rows
F2 <- F2_top_row / F2_bottom_row

#Display the combined plot
print(F2)
#Recommended dimensions: 2000 × 1000 pixels
